...
export function adaptWeather(r: unknown): T.WeatherCurrent {
  ...
  return {
    temp_min_c: num(w.min_temp_c),
    temp_max_c: num(w.max_temp_c),
    pressure_mbar: num(w.pressure_pa) / 100,
    solar_irradiance_w_m2: num(w.solar_irradiance_wm2),
    dust_opacity: num(w.dust_opacity),
    season: w.season,
  }
}
...
export function adaptEnergy(r: unknown): T.EnergyStatus {
  ...
  const bd = e.breakdown ?? ({} as RawEnergyStatus["breakdown"])
  return {
    solar_generation_kw: num(e.solar_generation_wh) / 1000,
    battery_pct: num(e.battery_pct),
    consumption: {
      heating_kw: num(bd.heating_wh) / 1000,
      lighting_kw: num(bd.lighting_wh) / 1000,
      water_kw: num(bd.water_recycling_wh) / 1000,
      nutrients_kw: num(bd.nutrient_pumps_wh) / 1000,
    },
    surplus_deficit_kw: e.deficit
      ? -Math.abs(num(e.total_consumption_wh) - num(e.solar_generation_wh)) / 1000
      : num(e.surplus_wh) / 1000,
  }
}
...
export function adaptGreenhouseEnv(r: unknown): T.ZoneEnvironment[] {
  ...
  return (g.zones ?? []).map((z) => ({
    zone_id: z.zone_id,
    temp_c: num(z.temp_c),
    humidity_pct: num(z.humidity_pct),
    co2_ppm: num(z.co2_ppm),
    par_light_umol: num(z.par_umol_m2s),
    photoperiod_hours: num(z.photoperiod_hours),
  }))
}

export function adaptWater(r: unknown): T.WaterStatus {
  const w = r as RawWaterStatus
  const capacity = num(w.reservoir_capacity_liters, 1)
  return {
    reservoir_level_pct: (num(w.reservoir_liters) / capacity) * 100,
    recycling_efficiency_pct: num(w.recycling_efficiency_pct),
    filter_health_pct: num(w.filter_health_pct),
    days_until_critical: num(w.days_until_critical),
    daily_consumption_l:
      num(w.daily_crew_consumption_liters) + num(w.daily_crop_consumption_liters),
    daily_recycled_l: num(w.daily_recycled_liters),
  }
}
...
export function adaptCrops(r: unknown): T.CropBatch[] {
  ...
  return (c.crops ?? []).map((crop) => {
    ...
    for (const s of crop.stress_indicators ?? []) {
      stressMap[s.type] = num(s.severity)
    }
    return {
      crop_id: crop.crop_id,
      crop_type: crop.type,
      zone_id: crop.zone_id,
      growth_pct: num(crop.growth_pct),
      health_pct: num(crop.health) * 100,
      ...
      days_to_harvest: Math.max(0, num(crop.expected_harvest_sol) - num(crop.current_sol)),
      yield_estimate_kg: num(crop.estimated_yield_kg),
      area_m2: num(crop.area_m2),
    }
  })
}
...
export function adaptNutrients(r: unknown): T.ZoneNutrients[] {
  ...
  return (n.zones ?? []).map((z) => ({
    zone_id: z.zone_id,
    ph: num(z.solution_ph),
    ec_ms_cm: num(z.solution_ec_ms_cm),
    nitrogen_ppm: num(z.nitrogen_ppm),
    phosphorus_ppm: num(z.phosphorus_ppm),
    potassium_ppm: num(z.potassium_ppm),
    calcium_ppm: num(z.calcium_ppm),
    magnesium_ppm: num(z.magnesium_ppm),
    stock_remaining_pct: num(n.nutrient_stock_remaining_pct),
    days_of_stock: num(n.days_of_stock_remaining),
  }))
}
...
export function adaptCrewMembers(r: unknown): T.CrewMember[] {
  ...
  return (c.members ?? []).map((m) => ({
    ...
    alive: m.alive,
    status: m.status,
    health_pct: num(m.health_pct),
    hydration_pct: num(m.hydration_pct),
    radiation_msv: num(m.cumulative_radiation_msv),
  }))
}

export function adaptCrewNutrition(r: unknown): T.CrewNutrition {
  const c = r as RawCrewNutrition
  const today = c.today ?? ({} as RawCrewNutrition["today"])
  const foodInventory: Record<string, number> = {}
  for (const [key, item] of Object.entries(c.food_inventory ?? {})) {
    foodInventory[key] = num(item.kg_remaining)
  }
  return {
    daily_calories_consumed: num(today.calories_consumed_kcal),
    daily_calories_target: num(today.calories_target_kcal),
    daily_protein_consumed_g: num(today.protein_consumed_g),
    daily_protein_target_g: num(today.protein_target_g),
    greenhouse_food_pct: num(today.from_greenhouse_pct),
    stored_food_pct: num(today.from_stored_food_pct),
    days_of_food_remaining: num(c.days_of_food_remaining),
...
export function adaptCrewHealth(r: unknown): T.CrewHealth {
  ...
  return {
    overall_health_pct: num(h.overall_health_pct),
    hydration_pct: num(h.hydration?.hydration_pct),
    radiation_cumulative_msv: num(h.radiation?.cumulative_msv),
    co2_impact: num(h.co2?.health_penalty_pct),
    starvation_level: num(h.starvation?.health_penalty_pct),
...
export function adaptCrises(r: unknown): T.ActiveCrisis[] {
  ...
  return (c.crises ?? [])
    ...
    .map((crisis) => ({
      ...
      threshold_breach: `${num(crisis.current_value).toFixed(1)} / ${num(crisis.threshold).toFixed(1)}`,
...
export function adaptScore(r: unknown): T.ScoreCurrent {
  ...
  const sc = s.scores ?? ({} as RawScoreCurrent["scores"])
  return {
    survival: num(sc.survival?.score),
    nutrition: num(sc.nutrition?.score),
    resource_efficiency: num(sc.resource_efficiency?.score),
    crisis_management: num(sc.crisis_management?.score),
    overall_score: num(sc.overall_score),
  }
}
