...
export function GreenhouseDetailView({ zoneId, crops, zone, nutrients, onBack }: GreenhouseDetailViewProps) {
  ...
  const avgHealth = zoneCrops.length > 0 ? Math.round(zoneCrops.reduce((s, c) => s + c.health_pct, 0) / zoneCrops.length) : 0;
  const avgGrowth = zoneCrops.length > 0 ? Math.round(zoneCrops.reduce((s, c) => s + c.growth_pct, 0) / zoneCrops.length) : 0;
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-mono text-lg tracking-[0.2em] text-foreground">GREENHOUSE {label}</span>
      </motion.div>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-10">
        <div className="flex flex-col items-center">
          ...
          {zoneCrops[3] && <div className={zoneCrops[1] || zoneCrops[2] ? "-mt-40" : ""}><CropBed crop={zoneCrops[3]} index={3} /></div>}
        </div>
      </div>
      ...
      <motion.div
        ...
        className="absolute top-20 left-4 bg-card/80 backdrop-blur-sm border border-border rounded-sm px-4 py-3 z-20 w-52 flex flex-col gap-3"
      >
        {/* Environment */}
        <div>
          <span className="label-aerospace block mb-2 text-primary">ENVIRONMENT</span>
          {zone && (
        ...
        {/* Water recycling */}
        <div>
          <span className="label-aerospace block mb-2 text-primary">WATER RECYCLING</span>
          {water ? (
            <div className="flex flex-col gap-1.5">
              <DataRow
                label="RESERVOIR"
                value={`${water.reservoir_level_pct.toFixed(1)}%`}
                color={water.reservoir_level_pct > 40 ? "text-primary" : water.reservoir_level_pct > 20 ? "text-amber-alert" : "text-destructive"}
              />
              <DataRow label="RECYCLING EFF." value={`${water.recycling_efficiency_pct.toFixed(1)}%`} />
              <DataRow label="FILTER HEALTH" value={`${water.filter_health_pct.toFixed(1)}%`} color={water.filter_health_pct > 50 ? "text-primary" : "text-amber-alert"} />
              <DataRow label="DAILY USE" value={`${water.daily_consumption_l.toFixed(1)} L`} />
              <DataRow label="RECYCLED" value={`${water.daily_recycled_l.toFixed(1)} L`} color="text-primary" />
              <DataRow
                label="DAYS LEFT"
                value={`${water.days_until_critical.toFixed(0)}`}
                color={water.days_until_critical > 30 ? "text-primary" : water.days_until_critical > 10 ? "text-amber-alert" : "text-destructive"}
              />
            </div>
          ) : (
        ...
        {/* Energy */}
        <div>
          <span className="label-aerospace block mb-2 text-primary">ENERGY</span>
          {energy ? (
            <div className="flex flex-col gap-1.5">
              <DataRow label="SOLAR GEN." value={`${energy.solar_generation_kw.toFixed(2)} kW`} color="text-primary" />
              <DataRow
                label="BATTERY"
                value={`${energy.battery_pct.toFixed(1)}%`}
                color={energy.battery_pct > 40 ? "text-primary" : energy.battery_pct > 20 ? "text-amber-alert" : "text-destructive"}
              />
      ...
      <motion.div
        ...
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.15 }}
        className="absolute top-20 bottom-16 right-4 bg-card/80 backdrop-blur-sm border border-border rounded-sm px-4 py-3 z-20 w-56 flex flex-col gap-2 overflow-y-auto"
      >
        {/* Zone summary header */}
        <div className="flex justify-between items-baseline mb-1">
          <span className="label-aerospace text-primary">CROPS</span>
          <span className="font-mono text-[10px] tabular-nums text-muted-foreground">{zoneCrops.length} beds · {totalYieldKg} kg est.</span>
        </div>

        {/* One card per crop */}
        {zoneCrops.map((c) => (
          <div key={c.crop_id} className="rounded border border-border bg-background/40 px-3 py-2 flex flex-col gap-1.5">
            {/* Header row */}
            <div className="flex justify-between items-baseline">
              <span className="font-mono text-[11px] font-bold text-foreground">{c.crop_type.toUpperCase()}</span>
              <span className="font-mono text-[10px] tabular-nums text-muted-foreground">{c.area_m2} M²</span>
            </div>
            ...
            <div>
              <div className="flex justify-between items-baseline mb-0.5">
                ...
                <span className="font-mono text-[10px] tabular-nums text-foreground">{c.growth_pct.toFixed(0)}%</span>
              </div>
              <div className="h-1.5 bg-secondary rounded-sm overflow-hidden">
                <div className="h-full rounded-sm bg-primary transition-all duration-500" style={{ width: `${c.growth_pct}%` }} />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-baseline mb-0.5">
                ...
                <span className={`font-mono text-[10px] tabular-nums ${c.health_pct >= 70 ? "text-primary" : c.health_pct >= 40 ? "text-amber-alert" : "text-destructive"}`}>{c.health_pct.toFixed(0)}%</span>
              </div>
              <div className="h-1.5 bg-secondary rounded-sm overflow-hidden">
                <div className={`h-full rounded-sm transition-all duration-500 ${c.health_pct >= 70 ? "bg-primary" : c.health_pct >= 40 ? "bg-amber-alert" : "bg-destructive"}`} style={{ width: `${c.health_pct}%` }} />
              </div>
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {c.stress_water > 0 && (
                <span className={`font-mono text-[9px] tabular-nums ${c.stress_water > 0.5 ? "text-destructive" : "text-amber-alert"}`}>
                  H₂O:{(c.stress_water * 100).toFixed(0)}%
                </span>
              )}
              {c.stress_nutrient > 0 && (
                <span className={`font-mono text-[9px] tabular-nums ${c.stress_nutrient > 0.5 ? "text-destructive" : "text-amber-alert"}`}>
                  NUT:{(c.stress_nutrient * 100).toFixed(0)}%
                </span>
              )}
              {c.stress_light > 0 && (
                <span className={`font-mono text-[9px] tabular-nums ${c.stress_light > 0.5 ? "text-destructive" : "text-amber-alert"}`}>
                  LIT:{(c.stress_light * 100).toFixed(0)}%
                </span>
              )}
              {c.stress_temp > 0 && (
                <span className={`font-mono text-[9px] tabular-nums ${c.stress_temp > 0.5 ? "text-destructive" : "text-amber-alert"}`}>
                  TMP:{(c.stress_temp * 100).toFixed(0)}%
                </span>
              )}
              {c.stress_water === 0 && c.stress_nutrient === 0 && c.stress_light === 0 && c.stress_temp === 0 && (
                <span className="font-mono text-[9px] text-primary">NO STRESS</span>
              )}
            </div>

            {/* Harvest & yield */}
            <div className="flex justify-between items-baseline pt-0.5 border-t border-border">
              <div className="flex items-baseline gap-1">
                <span className="label-aerospace">HARVEST</span>
                <span className="font-mono text-[10px] tabular-nums text-foreground">{c.days_to_harvest} SOLS</span>
              </div>
              <div className="flex items-baseline gap-1">
                <span className="label-aerospace">YIELD</span>
                <span className="font-mono text-[10px] tabular-nums text-primary">{c.yield_estimate_kg.toFixed(1)} KG</span>
              </div>
            </div>
          </div>
        ))}
        ...
        {nutrients && (
          <>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <DataRow label="STOCK" value={`${nutrients.stock_remaining_pct.toFixed(1)}%`} color={nutrients.stock_remaining_pct > 50 ? "text-primary" : "text-amber-alert"} />
              <DataRow label="STOCK DAYS" value={`${nutrients.days_of_stock.toFixed(0)}`} color={nutrients.days_of_stock > 30 ? "text-primary" : "text-amber-alert"} />
            </div>
          </>
        )}
      </motion.div>
...
function DataRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex justify-between items-baseline">
      ...
      <span className={`font-mono text-xs tabular-nums ${color || "text-foreground"}`}>{value}</span>
    </div>
  );
}
...
function CropBed({ crop, index }: { crop: CropBatch; index: number }) {
  ...
  return (
    ...
    >
      <img src={img} alt={`${crop.crop_type} bed`} className="w-[240px] h-auto" draggable={false} />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-background/85 border border-border rounded px-1.5 py-1 backdrop-blur-sm whitespace-nowrap z-10 text-center min-w-[90px]">
        <div className="font-mono text-[8px] text-foreground mb-0.5 leading-none">{crop.crop_type.toUpperCase()}</div>
        ...
        <div className="font-mono text-[7px] text-muted-foreground mt-0.5 leading-none">{crop.days_to_harvest}d</div>
      </div>
    </motion.div>
  );
}
