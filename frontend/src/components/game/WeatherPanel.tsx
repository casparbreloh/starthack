...
export function WeatherPanel({ weather }: WeatherPanelProps) {
  const stats = [
    ...
    { icon: Sun, label: "Solar", value: `${weather.solar_irradiance_w_m2}`, color: "" },
    {
      icon: CloudFog,
      label: "Dust",
      value: weather.dust_opacity < 0.5 ? "LOW" : weather.dust_opacity < 1 ? "MED" : "HIGH",
      color: weather.dust_opacity >= 1 ? "text-amber-alert" : "",
    },
  ]
  ...
  return (
    <div className="flex flex-col gap-2">
      {stats.map((s, i) => (
        ...
        >
          <s.icon className="h-4 w-4 text-muted-foreground" strokeWidth={2} />
