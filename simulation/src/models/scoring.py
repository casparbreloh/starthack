...
class ScoreSnapshot:
    current_sol: int = 0
    survival: dict = field(default_factory=lambda: {
        "crew_alive": True,
    ...
    resource_efficiency: dict = field(default_factory=lambda: {
        "water_efficiency_pct": 100.0,
        "energy_efficiency_pct": 100.0,
        "crop_waste_pct": 0.0,
        "score": 100,
    })
    crisis_management: dict = field(default_factory=lambda: {
...
class ScoringModel:
    ...
    def update(
        ...
        self.snapshot.survival["days_without_critical_deficit"] = self._consecutive_ok_sols
        self.snapshot.survival["score"] = 100 if crew_alive else 0

        # Nutrition
        kcal_ach = min(100.0, crew_kcal / CREW_DAILY_KCAL * 100.0) if CREW_DAILY_KCAL > 0 else 100.0
        prot_ach = min(100.0, crew_protein_g / CREW_DAILY_PROTEIN_G * 100.0) if CREW_DAILY_PROTEIN_G > 0 else 100.0
        ...
        nutrition_score = int((avg_kcal_ach * 0.5 + avg_prot_ach * 0.4 + micro_score * 10.0))
    ...
    def record_preventive_action(self) -> None:
        ...
        self.snapshot.crisis_management["preventive_actions_taken"] = self._preventive_actions
    ...
    def record_harvest(self, yield_kg: float) -> None:
        ...
        if self._total_harvest_kg > 0:
            self.snapshot.resource_efficiency["crop_waste_pct"] = round(
                self._crop_waste_kg / (self._total_harvest_kg + self._crop_waste_kg) * 100.0, 1
            )
